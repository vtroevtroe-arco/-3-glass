            ...
            {auth.isLoggedIn && (
              <>
                ...
                <NavItem 
                  ...
                  label="Lançar Nova OS" 
                  active={currentView === 'create-os'} 
                  onClick={() => navigate('create-os')} 
                />
                <NavItem 
                  icon={<FileText className="w-5 h-5" />} 
                  label="Histórico de OS" 
                  active={currentView === 'list-os'} 
            ...
            {auth.isLoggedIn ? (
              <div className="space-y-4">
                ...
                <button 
                  ...
                  className="flex items-center w-full gap-3 px-4 py-4 text-red-400 hover:bg-red-500/10 rounded-2xl transition-all duration-200 font-black text-sm uppercase tracking-widest"
                >
              ...
              <button 
                ...
                className="flex items-center w-full gap-4 px-6 py-5 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-2xl transition-all duration-300 font-black text-xs uppercase tracking-widest border border-gray-700/50"
              >
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <header className="md:hidden flex items-center justify-between p-5 bg-gray-900 border-b border-gray-800 sticky top-0 z-40 shadow-xl">
          ...
          <button 
            ...
            className="p-2.5 bg-gray-800 rounded-xl text-white active:scale-95 transition-transform"
          >
        ...
        <section className="flex-1 overflow-y-auto bg-gray-950 selection:bg-blue-600 selection:text-white">
          <div className="max-w-7xl mx-auto p-4 md:p-10 pb-32">
            {currentView === 'home' && (
              <Landing onStartChat={() => navigate('chat')} />
            )}
            {currentView === 'login' && <Login onLogin={handleLogin} />}
            {currentView === 'dashboard' && auth.isLoggedIn && (
              <Dashboard 
                onCreateOS={() => navigate('create-os')} 
                onViewOS={() => navigate('list-os')}
                onViewChat={() => navigate('chat')}
              />
            )}
            {currentView === 'create-os' && auth.isLoggedIn && (
              <OSForm 
                editingOS={editingOS} 
                onSave={() => navigate('list-os')} 
                onCancel={() => navigate('dashboard')} 
              />
            )}
            {currentView === 'list-os' && auth.isLoggedIn && (
              <OSList 
                onEditOS={handleEditOS} 
                onNewOS={() => navigate('create-os')} 
              />
            )}
...
const NavItem: React.FC<NavItemProps> = ({ icon, label, active, onClick }) => (
  <button
    onClick={onClick}
    className={`
      w-full flex items-center justify-between px-5 py-4 rounded-[1.25rem] transition-all duration-300 font-black text-xs uppercase tracking-widest
      ${active 
  ...
  >
    <div className="flex items-center gap-4">
      <div className={`${active ? 'text-white' : 'text-blue-500'} transition-colors`}>
        {icon}
      </div>
      <span>{label}</span>
    </div>
